...
from Xyloid.utils.misc import extract_user

from .help import add_command_help

flood = {}
profile_photo = "Xyloid/modules/cache/pfp.jpg"
...
async def block_user_func(client: Client, message: Message):
    ...
    if user_id == client.me.id:
        return await Kazu.edit("anda stress harap segera minum obat.")
...
async def unblock_user_func(client: Client, message: Message):
    ...
    if user_id == client.me.id:
        return await Xyloid.edit("anda stress harap segera minum obat.")
...
async def setname(client: Client, message: Message):
    ...
    if len(message.command) == 1:
        ...
        try:
            ...
            await Xyloid.edit(f"**Berhasil Mengubah Nama Telegram anda Menjadi** `{name}`")
...
async def set_bio(client: Client, message: Message):
    Xyloid = await edit_or_reply(message, "`Processing . . .`")
    if len(message.command) == 1:
        return await Xyloid.edit("Berikan teks untuk ditetapkan sebagai bio.")
    elif len(message.command) > 1:
        bio = message.text.split(None, 1)[1]
        try:
            await client.update_profile(bio=bio)
            await Xyloid.edit(f"**Berhasil Mengubah BIO anda menjadi** `{bio}`")
        except Exception as e:
            await Xyloid.edit(f"**ERROR:** `{e}`")
    else:
        return await Xyloid.edit("Berikan teks untuk ditetapkan sebagai bio.")
